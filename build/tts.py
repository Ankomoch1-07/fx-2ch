...
# BGM：OPとEDで同じ曲、本編(中間)は別曲。bgm/ からファイル名で選ぶ
# （名前に op/ed/open → OP-ED曲、main/honpen/body → 本編曲。無ければsorted順で1本目/2本目）
_bgm_files = sorted(glob.glob("remotion/public/bgm/*.mp3") + glob.glob("remotion/public/bgm/*.wav"))
def pick_bgm(keys, fallback_idx):
    if not _bgm_files:
        return None
    for f in _bgm_files:
        if any(k in os.path.basename(f).lower() for k in keys):
            return "bgm/" + os.path.basename(f)
    return "bgm/" + os.path.basename(_bgm_files[min(fallback_idx, len(_bgm_files) - 1)])
bgm_op_ed = pick_bgm(["op", "ed", "open"], 0)
bgm_main = pick_bgm(["main", "honpen", "body", "naka"], 1)
print(f"bgm: OP/ED={bgm_op_ed} / 本編={bgm_main}")
...
json.dump({"fps": FPS, "title": title, "opImages": op_images, "edImages": ed_images,
           ...
           "bgFrames": main_bg_frames, "opBgFrames": op_bg_frames,
           "bgmOpEd": bgm_op_ed, "bgmMain": bgm_main, "segments": segs},
